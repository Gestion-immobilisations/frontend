...
import { ETAT_BIEN_OPTIONS, TYPE_BIEN_LABELS, CHAMPS_SPECIFIQUES } from '../../utils/constants';
...
const EditBien = () => {
  ...
  const handleSubmit = async () => {
    if (!validateStep(1) || !hasChanges) return;
    ...
    try {
      ...
      const payload = {};
      const commonFields = ['date_acquisition', 'prix_acquisition', 'etat', 'localisation', 'description'];
  ...
  const renderSpecificFields = () => {
    ...
    return (
      <Grid container spacing={2}>
        {(CHAMPS_SPECIFIQUES[type] || []).map(field => {
          ...
          return (
            <Grid item xs={12} md={6} key={field}>
              <TextField
                ...
                disabled={field === 'type_bien'} // type_bien ne peut pas être modifié
              />
            </Grid>
          );
        })}
  ...
  if (!formData.type_bien) {
    return (
      <Box sx={{ p: 3 }}>
        ...
        <Button onClick={() => navigate('/biens')} sx={{ mt: 2 }}>Retour</Button>
      </Box>
    );
  }
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}>
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {TYPE_BIEN_LABELS[formData.type_bien]} • QR: {formData.qr_code}
          </Typography>
        </Box>
      </Box>
        ...
        {activeStep === 1 && (
          <>
            <Typography variant="subtitle2" gutterBottom>
              Modifier les caractéristiques de {TYPE_BIEN_LABELS[formData.type_bien]?.toLowerCase()}
            </Typography>
            <Divider sx={{ mb: 2 }} />
